...
var ElementTypeMap = {
    ...
    definitions: [DefinitionsElement, 'documentation types message portType binding service']
};
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns) {
    ...
    if (Array.isArray(obj)) {
        ...
        //parts.push(self.objectToXML("", name, "","",items));
    }
    else if (obj == false){
        parts.push('false');
    }
        ...
        for (var name in obj) {
            ...
            if(child["attribute"] != null) {
                for (var childAttribute in child["attribute"]) {
                  attributes += " " + childAttribute + "=\"" + child["attribute"][childAttribute] + "\"";
                }
              parts.push(['<',ns,name,xmlnsAttrib," ",attributes,'>'].join(''));
